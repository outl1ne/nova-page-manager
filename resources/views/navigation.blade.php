<h3 class="flex items-center font-normal text-white mb-6 text-base no-underline">
    ...
    <span class="sidebar-label">
        @lang("novaPageManager.navigationItemTitle")
    </span>
</h3>

<ul class="list-reset mb-8">
    @if(\Outl1ne\NovaPageManager\NPM::pagesEnabled())
        <li class="leading-wide mb-4 text-sm">
            <router-link
                :to="{ name: 'index', params: { resourceName: 'pages' } }"
                class="text-white ml-8 no-underline dim"
            >
                @lang("novaPageManager.navigationPagesTitle")
            </router-link>
        </li>
    @endif

    @if(\Outl1ne\NovaPageManager\NPM::regionsEnabled())
        <li class="leading-wide mb-4 text-sm">
            <router-link
                :to="{ name: 'index', params: { resourceName: 'regions' } }"
                class="text-white ml-8 no-underline dim"
            >
                @lang("novaPageManager.navigationRegionsTitle")
            </router-link>
        </li>
    @endif
</ul>
